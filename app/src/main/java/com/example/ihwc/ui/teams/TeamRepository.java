...
public class TeamRepository {
                ...
                if(response.isSuccessful()){
                    ...
                    for(int i=0;i<teams.getValue().size();i++) {
                        if (teams.getValue().get(i).getCountryShort().equals("LAT")) {
                            ...
                            Log.i("Flag", ""+R.drawable.lat_flag);
    ...
    /*public void setTeam(){
        TeamApi teamApi=ServiceGenerator.getTeamApi();
        Call<TeamResponse> call=teamApi.getTeam();
        call.enqueue(new Callback<TeamResponse>() {
            @EverythingIsNonNull
            @Override
            public void onResponse(Call<TeamResponse> call, Response<TeamResponse> response) {
                if(response.isSuccessful()){
                    team.setValue(response.body().getTeam());
                }
            }
            ...
            public void onFailure(Call<TeamResponse> call, Throwable t) {
                Log.i("Retrofit", "Team went wrong :(");
            }
        });
    }*/
